...
import { getVideosList } from "../redux/actions/getVideosList";
import { useDispatch, useSelector } from "react-redux";
import { getVideoById } from "../redux/actions/getVideoById";
...
function Home() {
  ...
  const videos = useSelector((state) => state.videos.videos.data);
  const Selectedvideo = useSelector((state) => state.Singlevideo.Singlevideo);

  async function handleShowVideo(id) {
    window.scrollTo(0, 0);
  ...
  useEffect(() => {
    if (Selectedvideo.data) {
      const res = Selectedvideo.data.video;
      setDisplayVideo(res);
    } else {
      ...
      <div className="listing-page-wrapper">
        <div className="container">
          <div className="upload-btn-wrapper">
            <Link to="/upload" className="red-btn">
              Upload Video
            </Link>
          </div>
              ...
              <div className="video-wrapper">
                {displayVideo ? (
                  <video
                    ...
                    autoPlay
                    controls
                    src={`http://localhost:5000/${displayVideo}`}
                  />
                ) : (
                  <div className="alert-area">
                    <p>Plese Select video</p>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="video-listing-wrapper">
                {videos?.map((p) => {
                  return (
                    <>
                      <div
                        className="video-list-card-wrapper shadow mb-4"
                        onClick={() => handleShowVideo(p._id)}
                      >
